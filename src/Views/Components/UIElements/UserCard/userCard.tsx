...
const UserCard = (props: Props): JSX.Element => {
  ...
  const addRoles = async (role: string): Promise<void> => {
    ...
    await fetch(`http://localhost:3001/users/${user.user_id}`, {
      method: "POST",
      headers: {
    ...
    <div className="mb-4 mr-4 flex-grow">
      <div className="shadow-lg rounded-2xl p-4 w-full bg-white">
        <div className="flex justify-between mb-4">
          ...
          {edit ? (
            <CheckCircleIcon
              className="mb-2 h-10 w-10 bg-green-50 rounded-full text-green-500 p-1"
              onClick={() => {
                setEdit(false);
              }}
            />
        ...
        <div className="flex justify-between items-center">
                      ...
                      <Switch
                        ...
                        className={`${
                          activeRoles[role].active
                            ? "bg-green-500"
                            : "bg-purple-300"
                        }  flex items-center h-6 w-10 rounded-full`}
                      >
                        ...
                        <span
                          ...
                          className={`${
                            activeRoles[role].active
                              ? "translate-x-4"
                              : "translate-x-0"
                          } flex items-center justify-center w-6 h-6 transform bg-white rounded-full transition-all border border-${
                            activeRoles[role].active
                              ? "green-500"
                              : "purple-300"
                          }`}
          ...
          <img
            alt="profile"
            src={user.picture}
            className="ml-4 block h-auto w-auto rounded-full max-h-36"
          />
        </div>
